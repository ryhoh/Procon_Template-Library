...
// 素因数分解
map<int, int> primeFactors(long long N, vector<int> primes) {
    // check for N
...
long long gcd(long long a, long long b) {
    if (a == 0 && b == 0) throw invalid_argument("gcd of 0 and 0");

    if (a < b) return gcd(b, a);
    assert(a >= b);
    if (b < 0) throw invalid_argument("gcd of minus value");
...
long long lcm(long long a, long long b) {
    if (a < 0 || b < 0) throw invalid_argument("lcm of minus");
    if (a == 0 || b == 0) throw invalid_argument("lcm of 0");
...
long long pow_mod(long long a, long long b, long long mod) {
    if (a < 0 || b < 0) throw invalid_argument("pow_mod of minus values");
    if (b == 0) return 1;

    long long p = pow_mod(a, b/2, mod);
    if (b % 2 == 0) {
        return p * p % mod;
    } else {
